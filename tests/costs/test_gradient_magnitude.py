...
from src.costs import GradientMagnitude
...
def test_calculate_store_history():
    size = (260, 346)
    events = utils.generate_events(1000, size[0], size[1], tmin=0.1, tmax=0.9).astype(np.float32)
    events_torch = torch.from_numpy(events)
    imager = EventImageConverter(size)

    cost = GradientMagnitude(direction="minimize", store_history=True)
    # Calculate numpy
...
def test_calculate_not_store_history():
    ...
    cost = GradientMagnitude(direction="minimize", store_history=False)
    # Calculate numpy
    count = imager.create_image_from_events_numpy(events, "bilinear_vote", weight=1.0, sigma=0)
    _ = cost.calculate({"iwe": count, "omit_boundary": True})
...
def test_calculate_blur_is_small(direction, is_small):
    size = (10, 40)
    imager = EventImageConverter(size)
    cost = GradientMagnitude(direction=direction, store_history=False)
    ...
    grad_blur = cost.calculate({"iwe": imager.create_iwe(events), "omit_boundary": False})
    events = np.array(
        [
            [5.0, 10.0],
            [5.0, 10.0],
    ...
    grad_sharp = cost.calculate({"iwe": imager.create_iwe(events), "omit_boundary": False})

    assert (grad_blur < grad_sharp) == is_small


def test_calculate_np_torch():
    size = (10, 20)
    imager = EventImageConverter(size)
    cost = GradientMagnitude(direction="natural", store_history=False)
